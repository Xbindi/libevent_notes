 ...
 * 客户端：发出请求，等待回应
 * 服务端：解析请求，发出回应
 * 测试方法：浏览器输入ip:8080即可获得index.html页面
 * 		   浏览器输入ip:8080/123.png即可获得图片页面
 * 		   浏览器输入ip:8080/xxx.zip即可获得压缩包文件并下载
 * *****************************************************/
...
void evhttp_cb(struct evhttp_request* request, void *arg)
{
	...
	if(strcmp(uri, "/") == 0)
	{
		filepath += DEFAULTINDEX;//默认加入首页文件
	}
	//消息报头
	...
	for(;;)
	{
		int len = fread(buf, 1, sizeof(buf), fp);//回复index.html即fp指向的文件
		if(len <= 0) break;
