/*************************************************************************
 *     > File Name: first_conf.cpp
 *     > Created Time: 2020年12月26日 星期六 21时05分01秒
 * linux环境：
...
int main()
{
	...
	//配置上下文   创建时就立刻写释放函数再写配置项
	event_config* conf = event_config_new();

	//get支持的网络模式:selcct poll epoll
	...
	//set特征, EV_FEATURE_ET 设置支持边缘触发 
	//event_config_require_features(conf, EV_FEATURE_ET | EV_FEATURE_FDS);
	//设置了EV_FEATURE_FDS后其他特征就无法设置 且不支持epoll
	//event_config_require_features(conf, EV_FEATURE_FDS);


	//默认支持epoll,但是通过event_config_avoid_method来取消epoll和poll的支持,设置后仅支持select
	...
	else
	{
		...
		evconnlistener* ev = evconnlistener_new_bind(base, listen_cb, base, 10, 
								LEV_OPT_CLOSE_ON_FREE | LEV_OPT_REUSEABLE, (sockaddr*)&sin, sizeof(sin)); 
