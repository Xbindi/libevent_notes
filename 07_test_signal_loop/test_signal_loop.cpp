 ...
 *			EVLOOP_ONCE等待一个事件运行，直到没有活动事件则退出
 *			EVLOOP_NONBLOCK有活动事件就处理，没有就返回
 *			EVLOOP_NO_EXIT_ON_EMPTY没有注册事件也不返回，用于事件后期多线程添加
 *	事件退出循环也有对应的2种方式
 *	1 event_base_loopbreak(base);  执行完当前的事件任务就退出，如果无操作则立即退出
 *	2 timeval t = {3,0};
 *	  event_base_loopexit(base, &t)  
 *    运行完所有激活事件的回调才退出，事件循环没有运行时也要运行一次在退出，该写法表示至少运行3秒才退出
 *********************************************************************************/
#include <iostream>
...
static void Ctrl_C(int sock, short which, void* arg)
{
	...
	//运行完所有的事件再退出,事件循环没有运行时也要等运行一次在退出, 至少运行3秒才退出
	timeval t = {3,0};
	event_base_loopexit(base, &t);
}
...
int main(int argc, const char** argv)
{
	...
	//EVLOOP_NONBLOCK有活动事件就处理，没有就返回0
	//while(!isexit){
	//	event_base_loop(base, EVLOOP_NONBLOCK);
	//}
	
	//EVLOOP_NO_EXIT_ON_EMPTY没有注册事件也不返回，用于事件后期多线程添加
	//event_base_loop(base, 0);//效果等同于event_base_dispatch(base)
	event_base_loop(base, EVLOOP_NO_EXIT_ON_EMPTY);
		
	cout << "test_signal_loop end!" << endl;
	event_free(csig);
