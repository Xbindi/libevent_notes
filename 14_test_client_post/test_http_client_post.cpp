...
void http_client(struct evhttp_request* req, void* ctx)
{
	...
	if(fp)
		fclose(fp);
	event_base_loopbreak(bev);//获取一次请求后退出
}


int TestPostHttp()
{
	std:: cout << "test client post!\n";
	//创建libevent的上下文
	...
	//生成请求信息POST 需要12_test_http_server服务端程序进行配合测试
	...
	evbuffer_add_printf(output, "tzj=%d&b=%d", 100, 234567);
